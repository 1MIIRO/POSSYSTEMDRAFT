    ...
    try:
        ...
        cursor.execute("""
            SELECT u.user_id, u.user_name, u.personal_name, u.job_desc,
                   g.USER_GROUPS
            FROM user u
            LEFT JOIN user_with_group uwg ON u.user_id = uwg.USER_id
            LEFT JOIN user_groups g ON uwg.USER_GROUPS_number = g.USER_GROUPS_number
        """)
...
def inventory_status():
    ...
    SELECT
        ...
        p.good_name,
        COALESCE(
            (SELECT pqc.current_quantity
             FROM product_quantity_current pqc
             WHERE pqc.good_number = p.good_number
             ORDER BY pqc.updated_at DESC
             LIMIT 1),
            (SELECT psi.initial_quantity
             FROM product_stock_initial psi
             WHERE psi.good_number = p.good_number
             LIMIT 1),
            0
        ) AS current_quantity,
        CASE
...
IMAGES_FOLDER = os.path.join('static', 'edit_images')
...
def list_food_images():
    ...
    return jsonify(files)   # <-- FIXED (return only filenames)
